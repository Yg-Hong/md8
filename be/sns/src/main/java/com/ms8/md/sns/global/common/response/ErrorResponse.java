package com.ms8.md.sns.global.common.response;
...
public class ErrorResponse {
    ...
    @Schema(description = "에러 구분 코드", example = "G009")
    private String divisionCode;        // 에러 구분 코드
    @Schema(description = "에러 메시지", example = "Not Found Exception")
    private String resultMsg;           // 에러 메시지
    @Schema(description = "상세 에러 메시지", example = "List 형태의 에러 메시지")
    private List<FieldError> errors;    // 상세 에러 메시지
    @Schema(description = "에러 이유", example = "에러 이유")
